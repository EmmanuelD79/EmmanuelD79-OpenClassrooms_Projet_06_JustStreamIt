...
async function getMovies (data, nameCategorie) {
	...
	for (const RESULT_MOVIE of data.results) {
		...
		MOVIE_ITEM.onclick = function (){showModal(RESULT_MOVIE.id)}
	...
	document.getElementById("pre_" + CATEGORIE.id).onclick = function() {goPrev(CATEGORIE.id)}
	document.getElementById("next_" + CATEGORIE.id).onclick = function() {goNext(CATEGORIE.id)}
...
async function getBestMovie (data) {
	...
	bestImg.onclick = function (){showModal(movieInfo.id)}
